  ...
  function scanDirectory(dir: string) {
    try {
      ...
      for (const item of items) {
        ...
        // Skip node_modules, .git, .svelte-kit, dist, build directories
        if (stat.isDirectory() && !item.startsWith('.') && 
            !['node_modules', 'dist', 'build', '.svelte-kit', 'out'].includes(item)) {
...
export function createEditorHandle(config: EditorConfig = { contentDir: './', framework: 'sveltekit' as const }): Handle {
    ...
    try {
      ...
      if (pathname === '/api/_arjun_edit/ping') {
        return new Response(JSON.stringify({
          success: true,
          port: 5173, // SvelteKit default dev port
          contentDir: './',
          integrated: true
        }), {
          headers: { 'content-type': 'application/json' }
      ...
      console.error('Arjun Editor handle error:', error);
    }
...
// Generate dashboard HTML for SvelteKit
...
<body>
    <div class="container">
        <div class="header">
            <h1>arjun-editor</h1>
            <p class="subtitle">Integrated with your SvelteKit dev server</p>
        </div>
        
        <div class="integration-info">
            🔗 Running on the same port as your SvelteKit app - no separate server needed!
        </div>
...
export function mountMarkdownEditor(config: EditorConfig = { contentDir: './', framework: 'sveltekit' as const }) {
  // Only run in development
  if (process.env.NODE_ENV === 'production') {
    return;
  }
  
  console.log('✨ arjun-editor: Integrated with your SvelteKit dev server');
  console.log('📂 Visit http://localhost:5173/_arjun_edit to see all markdown files');
  console.log('🎯 Edit any file at http://localhost:5173/_arjun_edit/[file-path]/_edit');
} 